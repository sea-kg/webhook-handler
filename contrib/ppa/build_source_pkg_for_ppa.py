...
def clear_all():
    print( " -> Clear all")
    if os.path.exists('./webhook-handler'):
        shutil.rmtree('./webhook-handler')
    print( " -> DONE")
    print( " -> Cleanup previous ppa packages")
    onlyfiles = [f for f in os.listdir('./') if os.path.isfile(os.path.join('./', f))]
    for f in onlyfiles:
        m = re.search(r'^webhook-handler_(\d+\.\d+\.\d+)-ppa-.*(\.orig\.tar\.gz|source\.changes|_source\.build|_source.ppa.upload|\.tar\.gz|_source\.buildinfo|\.dsc)$', f)
        if m:
            print('Remove file ' + f)
            os.remove(f)
...
shutil.copytree('../../src', './webhook-handler/src', symlinks=False, ignore=None)
shutil.copytree('../../src.wsjcpp', './webhook-handler/src.wsjcpp', symlinks=False, ignore=None)
shutil.copy2('../../CMakeLists.txt', './webhook-handler/CMakeLists.txt')
shutil.copy2('../../VERSION', './webhook-handler/VERSION')
shutil.copy2('../../LICENSE', './webhook-handler/LICENSE')
shutil.copytree('./debian', './webhook-handler/debian', symlinks=False, ignore=None)
shutil.copytree('./install-files', './webhook-handler/install-files', symlinks=False, ignore=None)
...
changelog="./webhook-handler/debian/changelog"
f = open(changelog,'w')
li_count = 0
for li in changelog_list:
    if li_count != 0:
        ...
        f.write("\n")
    li_count = li_count + 1
    f.write("webhook-handler (" + li['version'] + "-" + ppa_name_ + ") " + dist_name_ + "; urgency=low\n\n")
...
os.system("cd ./ && tar -acf webhook-handler_" + current_version + "-" + ppa_name_ + ".orig.tar.gz webhook-handler")
os.system("cd ./webhook-handler && debuild -S -sa")
...
if ask_upload_ == "y":
    os.system("dput ppa:sea5kg/webhook-handler " + dput_filename)
